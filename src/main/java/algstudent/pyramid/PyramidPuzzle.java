...
class PyramidBoard extends Node {
	...
	/**
	 * Inserts the values of a line from the pyramid It is call once per every
	 * row of the pyramid to initialize all the values
	 * 
	...
	public void insertValues(String[] values, int row) {
		for (int i = 0; i < row + 1; i++) { // in row 0, 1 value; in row 1, 2
											// values...
			if (values[i].equals("*"))
				board[row][i] = 0;
			else
	...
	/**
	 * To get the children of the current node. They point to their parent
	...
	public ArrayList<Node> expand() {
		...
		for (int k = 0; k < 10; k++) {
			...
			temp = new PyramidBoard(newBoard, depth + 1, this.getID(), row,
					column);
			result.add(temp);
		}
	 ...
	 * @param parentID
	 */
	public PyramidBoard(int[][] board, int depth, UUID parentID, int row,
			int column) {
		this.board = board;
		this.depth = depth;
