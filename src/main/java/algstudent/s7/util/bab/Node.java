package algstudent.s7.util.bab;

import java.util.ArrayList;
import java.util.UUID;

/**
 * To represent the different states of a problem in the graph For each problem,
 * we should extend this class with specific information We also need to compare
 * Nodes because it is the way to compare them in the priority queue
 * 
 * @author viceg
 */
public abstract class Node implements Comparable<Node> {
	protected int depth; // Number of moves made so far (is equal to the number
							// of nodes developed) on this branch
