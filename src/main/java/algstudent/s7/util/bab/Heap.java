...
public class Heap {
	...
	public void insert(Node node) {
		if (!nodeRepeated(node)) { // Not repeat used nodes and avoid infinite
									// loops (e.g., in the puzzle problem)
			nodes.add(node);
		}
	}
	...
	public Node extractBestNode() {
		...
		usedNodes.put(node.getID(), node); // We save it because it can be part
											// of the solution
	...
	/**
	 * Extracts the nodes from a specific node (e.g., the solution) to the root
	 * node
	 * 
